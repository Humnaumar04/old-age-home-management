        ...
        <div>
            ...
            <div class="flex items-center space-x-3 mb-8 bg-[#255A66] p-3 rounded-xl">
                <div class="w-10 h-10 rounded-full bg-[#D1884F] flex items-center justify-center font-bold text-white">
                    @php
                    $name = auth()->user()->name ?? 'Family Member';
                    $initials = preg_replace('/[^A-Z]/', '', ucwords(strtolower($name)));
                    echo substr($initials, 0, 2);
                    @endphp
                </div>
                <div>
                    <h4 class="font-semibold text-sm">{{ auth()->user()->name ?? 'Family Name' }}</h4>
                    <p class="text-xs text-teal-300">Family Member</p>
                </div>
            </div>

            <!-- Fetch Unread Admin Messages Count -->
            @php
            $unreadAdminCount = \App\Models\Message::where('user_id', auth()->id())
            ...
            <nav class="space-y-2 mb-6">
                <a href="{{ route('family.dashboard') }}" class="flex items-center space-x-3 px-4 py-3 rounded-xl font-medium transition {{ request()->is('family/dashboard*') ? 'bg-[#255A66] text-white shadow-sm' : 'text-teal-100 hover:bg-[#2A6673]' }}">
                    <span>👤</span> <span>Resident Profile</span>
                </a>

                <a href="{{ url('/family/communication') }}" class="flex items-center justify-between px-4 py-3 rounded-xl font-medium transition {{ request()->is('family/communication*') ? 'bg-[#255A66] text-white shadow-sm' : 'text-teal-100 hover:bg-[#2A6673]' }}">
                    <div class="flex items-center space-x-3">
