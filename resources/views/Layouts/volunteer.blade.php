...
<head>
    ...
    <title>@yield('title', 'Volunteer Dashboard') - Old Age Home</title>
        ...
        <div>
            ...
            <div class="flex items-center space-x-3 mb-8 bg-[#255A66] p-3 rounded-xl">
                <div class="w-10 h-10 rounded-full bg-[#D1884F] flex items-center justify-center font-bold text-white">
                    @php
                    $name = auth()->user()->name ?? 'Sara Malik';
                    $initials = preg_replace('/[^A-Z]/', '', ucwords(strtolower($name)));
                    echo substr($initials, 0, 2) ?: 'SM';
                    @endphp
                </div>
                <div>
                    <h4 class="font-semibold text-sm">{{ auth()->user()->name ?? 'Sara Malik' }}</h4>
                    <p class="text-xs text-teal-300">Volunteer</p>
                </div>
            </div>

            <!-- Navigation Links -->
            <nav class="space-y-2 mb-6">
                <a href="#" class="flex items-center space-x-3 px-4 py-3 rounded-xl font-medium transition bg-[#255A66] text-white shadow-sm">
