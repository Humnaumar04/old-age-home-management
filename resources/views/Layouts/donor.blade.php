...
<body class="bg-[#F8F6F0] text-gray-800 flex flex-col md:flex-row h-screen overflow-hidden" x-data="{ sidebarOpen: false }">
    ...
    <aside :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'"
        class="fixed md:static inset-y-0 left-0 z-40 w-64 bg-[#1E4C56] text-white flex flex-col justify-between p-6 flex-shrink-0 overflow-y-auto transition-transform duration-300 ease-in-out md:translate-x-0 h-full">
        <div>
            ...
            <div class="flex items-center space-x-3 mb-8 bg-[#255A66] p-3 rounded-xl">
                ...
                <div>
                    <h4 class="font-semibold text-sm">{{ auth()->user()->name ?? 'Donor Name' }}</h4>
                    <p class="text-xs text-teal-300">Donor</p>
                </div>
            </div>

            <nav class="space-y-2 mb-6">
                <a href="{{ route('donor.dashboard') }}" class="flex items-center space-x-3 px-4 py-3 rounded-xl font-medium transition {{ request()->is('donor*') && !request()->is('donor/make-donation*') && !request()->is('donor/my-donations*') ? 'bg-[#255A66] text-white shadow-sm' : 'text-teal-100 hover:bg-[#2A6673]' }}">
                    <span>📦</span> <span>Requirements</span>
                </a>
                <a href="{{ route('donor.make-donation') }}" class="flex items-center space-x-3 px-4 py-3 rounded-xl font-medium transition {{ request()->is('donor/make-donation*') ? 'bg-[#255A66] text-white shadow-sm' : 'text-teal-100 hover:bg-[#2A6673]' }}">
                    <span>❤️</span> <span>Make Donation</span>
                </a>
                <a href="{{ route('donor.my-donations') }}" class="flex items-center space-x-3 px-4 py-3 rounded-xl font-medium transition {{ request()->is('donor/my-donations*') ? 'bg-[#255A66] text-white shadow-sm' : 'text-teal-100 hover:bg-[#2A6673]' }}">
                    <span>📋</span> <span>My Donations</span>
                </a>
            </nav>
        </div>
    ...
    <!-- PAGE CONTENT -->
    <main class="flex-1 overflow-y-auto overflow-x-auto p-4 md:p-12">
        @yield('content')
    </main>
    @stack('scripts')
</body>
