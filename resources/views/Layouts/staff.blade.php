...
<body class="bg-[#F8F6F0] text-gray-800 flex flex-col md:flex-row h-screen overflow-hidden" x-data="{ sidebarOpen: false }">
    ...
    <aside :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'"
        ...
        <div>
            ...
            <nav class="space-y-2">
                <a href="{{ route('staff.dashboard') }}" class="flex items-center space-x-3 px-4 p-3 rounded-xl font-medium transition {{ request()->routeIs('staff.dashboard') ? 'bg-[#255A66] text-white shadow-sm' : 'text-teal-100 hover:bg-[#2A6673]' }}">
                    <span>📊</span> <span>Dashboard</span>
                </a>
                <a href="#" class="flex items-center space-x-3 px-4 p-3 rounded-xl font-medium transition {{ request()->is('staff/update-health*') ? 'bg-[#255A66] text-white shadow-sm' : 'text-teal-100 hover:bg-[#2A6673]' }}">
                    <span>🩺</span> <span>Update Health</span>
                </a>
                <a href="/staff/report-emergency" class="flex items-center space-x-3 px-4 p-3 rounded-xl font-medium transition {{ request()->is('staff/report-emergency*') ? 'bg-[#255A66] text-white shadow-sm' : 'text-teal-100 hover:bg-[#2A6673]' }}">
